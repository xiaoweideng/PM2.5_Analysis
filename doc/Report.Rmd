...
```{r, echo = FALSE}
kable(head(BJ_data)) %>% kable_styling(position = "center")
```
...
```{r, echo = FALSE}
kable(head(SH_data)) %>% kable_styling(position = "center")
```
...
kable(head(BJSH_data)) %>% kable_styling(position = "center")
```
...
plyr::rename(summarized_data, c("n" = "number of sample",
                          "mean_PM" = "mean",
